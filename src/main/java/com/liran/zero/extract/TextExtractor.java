package com.liran.zero.extract;
...
public class TextExtractor {
	...
	public static class ExtractContent {
		...
		public ExtractContent(String title, String content) {
	...
	public static boolean isExtract(String url) {
		if (url == null || url.length() == 0) {
			return false;
		}
		return isContentURL(url) && isLegalURL(url);
	}

	public static ExtractContent extractHTML(String htmlText) {

		String title = extractTitle(htmlText);
		String content = "";
		htmlText = preProcess(htmlText);
		if (!isContentPage(htmlText)) {
			return new ExtractContent(title, "*推测您提供的网页为非主题型网页，目前暂不处理！:-)");
		}
		...
		if (result.replaceAll("\\s+", "").length() < 100)
			...
			content = replaceSpecialChar(result);

		StringBuilder buffer = new StringBuilder();
		for (String line : content.split("\n")) {
			line = line.trim();
			if (line.length() != 0) {
				int length = line.length() > 78 ? 78 : line.length() - 1;
				int width = 0;
				for (int i = 0; i < line.length() && length != 0; i = i
						+ length) {
					width = i == 0 ? 78 : 80;
					length = line.length() > (i + width) ? width : line
							.length() - i - 1;
					if (length == 0) {
						break;
					}
					if (i == 0) {
						buffer.append("  " + line.substring(i, i + length)
								+ "\n");
					} else {
						buffer.append(line.substring(i, i + length) + "\n");
					}
		...
		return new ExtractContent(title, buffer.toString());
	}
	 ...
	 *            the html text
	 */
	public static String extractTitle(String htmlText) {
	 ...
	 */
	private static List<Integer> lineBlockDistribute(List<String> lines) {
	...
	public static void main(String[] args) throws ZeroException {
		...
		System.out
				.println(extractHTML(
						fetcher.getHtml(
								new RequestParams(
										"http://laotie.baijia.baidu.com/article/19258"))
